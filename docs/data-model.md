...
| dealerMode           | `"host"                            | "draw_compare"`                     |
| dealerId             | string                             | 庄家玩家 ID                             |
| dealerDrawResults    | MapplayerId, Card                  | null                                |
| status               | `"waiting"                         | "choosing_dealer"                   |
| players              | Player[]                           | 玩家列表（按加入时间排序，房主第一，AI 排最后）           |
| discardPile          | Card[]                             | 弃牌堆                                 |
| chosenColor          | `"red"                             | "yellow"                            |
| currentPlayerIndex   | number                             | 当前出牌人在玩家列表中的索引                      |
| direction            | `1                                 | -1`                                 |
| pendingDraw          | `{ count: number, type: "draw_two" | "wild_draw_four"                    |
| drawPileCount        | number                             | 摸牌堆剩余张数（不暴露具体牌）                     |
| handCounts           | MapplayerId, number                | 各玩家手牌数量（不暴露内容）                      |
| scores               | MapplayerId, number                | 各玩家累计得分                             |
