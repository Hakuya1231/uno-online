...
| 质疑    | POST | `/api/game/challenge`       | `{ roomId }`                          | `{ hand, result }` | 该玩家被 +4                     | 读取上家手牌判断合法性（判定依据为 `pendingDraw.type==="wild_draw_four"` 时携带的 `pendingDraw.sourceColor`）；质疑成功：上家摸 4 张；质疑失败：质疑者摸 6 张。更新相关手牌、handCounts、drawPileCount，并清空 pendingDraw                                                           |
| 开始下一局 | POST | `/api/game/next-round`      | `{ roomId }`                          | `{}`               | 仅房主，状态为 finished            | 保留 players 和 scores，重置牌堆、手牌、currentRound+1；使用 `roundWinnerId` 作为上一局胜者并设为庄家（dealerId=roundWinnerId），清空 roundWinnerId，status=dealing                                                                                                                               |
